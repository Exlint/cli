...
export const adjustLocalToPlugins = async (projectId: string, ...libs: ILibrary[]) => {
	if (libs.length === 0) {
		return;
	}
	...
	if (libs.includes('stylelint')) {
		const stylelintXmlFilePath = path.join(process.cwd(), '.idea', 'stylesheetLinters', 'stylelint.xml');
		...
		writePluginsConfigurationsPromises.push(fs.outputFile(stylelintXmlFilePath, stylelintXmlFileContent));
	...
	if (shouldOverridePlugins) {
		...
		writePluginsConfigurationsPromises.push(fs.outputFile(workspaceXmlFilePath, workspaceXmlFileContent));
	}
